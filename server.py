"""
Poker Combos Trainer v5 - Production (Render)
"""
...
RANGES = {
    ...
    'CO': {
        'open': {
            ...
            'KQs','KJs','KTs','K9s','K8s',
            'QJs','QTs','Q9s','Q8s',
            'JTs','J9s','J8s','T9s','T8s','98s','97s','87s','86s','76s','65s',
...
SITUATIONS = [
    {'pos':'UTG', 'sit':'open',        'label':'UTG open'},
    {'pos':'MP',  'sit':'open',        'label':'MP open'},
    {'pos':'CO',  'sit':'open',        'label':'CO open'},
    {'pos':'BTN', 'sit':'open',        'label':'BTN open'},
    {'pos':'SB',  'sit':'open',        'label':'SB open'},
    {'pos':'SB',  'sit':'3bet_vs_BTN', 'label':'SB 3bet vs BTN'},
    {'pos':'BB',  'sit':'call_vs_BTN', 'label':'BB call vs BTN'},
    {'pos':'BB',  'sit':'3bet_vs_BTN', 'label':'BB 3bet vs BTN'},
    {'pos':'BB',  'sit':'call_vs_UTG', 'label':'BB call vs UTG'},
]
    ...
    for h in range_set:
        if len(h)==2 and h[0]==h[1]:
            ...
            for s1,s2 in combinations(SUITS,2):
                hands.append((r,r,s1,s2))
        elif len(h)==3 and h[2]=='s':
            r1,r2 = h[0],h[1]
            for s in SUITS:
                hands.append((r1,r2,s,s))
        elif len(h)==3 and h[2]=='o':
            r1,r2 = h[0],h[1]
            for s1 in SUITS:
                for s2 in SUITS:
                    if s1!=s2:
                        hands.append((r1,r2,s1,s2))
    return hands
...
def deal_question():
    ...
    sit_info = random.choice(SITUATIONS)
    pos,sit,sit_label = sit_info['pos'],sit_info['sit'],sit_info['label']
    range_set = RANGES[pos][sit]
    ...
    for _ in range(400):
        ...
        if hero_hand_key(hero) not in range_set: continue
    ...
    return {
        ...
        'board_size':board_size,'street':street,
        'hero_hand':hero_hand_name,'hero_hand_key':hero_hand_key(hero),
        'situation':sit_label,'total_range_combos':len(valid_hands),
        'total_beating':total_beating,'fields':fields,'grid':grid,
    }
...
.main{width:100%;max-width:900px;margin-top:24px;display:flex;flex-direction:column;gap:12px;}
...
.b-info{background:#2a2a2a;color:var(--text2);border:1px solid var(--border);font-size:10px;}
.main-question{background:linear-gradient(135deg,#0f1a2e,#1a0f2e);border:1px solid #2a3f6a;border-radius:12px;padding:16px 18px;margin-bottom:14px;}
.mq-label{font-size:11px;color:var(--text3);text-transform:uppercase;letter-spacing:.1em;margin-bottom:6px;}
.mq-text{font-family:'Syne',sans-serif;font-size:17px;font-weight:800;color:var(--text);line-height:1.4;}
.mq-sub{font-size:12px;color:var(--text2);margin-top:4px;}
.range-toggle button{height:30px;padding:0 12px;border-radius:8px;font-size:11px;font-family:'Syne',sans-serif;font-weight:700;border:1px solid var(--border);background:var(--surface2);color:var(--text);cursor:pointer;margin-bottom:8px;}
.range-toggle button:hover{background:var(--border);}
.grid-wrap{overflow-x:auto;}
table.rgrid{border-collapse:collapse;width:100%;}
table.rgrid td{width:7.69%;aspect-ratio:1;border:0.5px solid #1a1e2a;font-size:8.5px;font-weight:500;text-align:center;vertical-align:middle;color:var(--text3);background:var(--surface2);padding:0;line-height:1;}
...
.fields-title{font-size:12px;color:var(--text2);margin-bottom:10px;line-height:1.5;}
.fields-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px;}
.field-row{display:flex;align-items:center;gap:8px;background:var(--surface2);border:1px solid var(--border);border-radius:9px;padding:9px 12px;}
.field-label{flex:1;font-size:12px;color:var(--text2);}
.field-input{width:68px;height:38px;background:#fff;border:1.5px solid #999;border-radius:7px;color:#111;font-family:'Syne',sans-serif;font-size:17px;font-weight:800;text-align:center;outline:none;transition:border-color .15s;}
...
.field-res{width:22px;text-align:center;font-size:13px;}
.field-res.ok{color:var(--green);}.field-res.ko{color:var(--red);}
.field-ans{font-size:11px;color:var(--red);min-width:22px;text-align:right;display:none;font-weight:700;}
...
.summary{margin-top:12px;padding:12px 16px;border-radius:10px;display:none;}
.summary.show{display:block;}
.summary.all-ok{background:#0a2018;border:1px solid #1a4a30;}
.summary.partial{background:#1a1400;border:1px solid #3a2a00;}
.sum-title{font-family:'Syne',sans-serif;font-size:16px;font-weight:800;margin-bottom:4px;}
...
<div style="text-align:center">
  ...
  <p class="sub">Combien de combos de la range te battent ?</p>
</div>
<div class="main">
  <div class="stats">
  ...
  <div class="card">
    <div class="top-row"><div class="lbl">Question</div><button class="btn btn-n" onclick="newQ()">Nouvelle ↺</button></div>
  ...
  <div class="card">
    ...
    <div class="help">
      <b>Objectif</b> : compter uniquement les combos de la range adverse qui font une main plus forte que la tienne.<br>
...
let state={},stats={ok:0,tot:0,streak:0,fok:0,ftot:0},answered=false,gridVisible=false;
...
function buildGrid(grid,heroKey){let html='<table class="rgrid">';for(let i=0;i<13;i++){html+='<tr>';for(let j=0;j<13;j++){const r1=RANKS13[i],r2=RANKS13[j];const pp=i===j,suited=i<j;let label=pp?r1+r2:(suited?r1+r2+'s':r2+r1+'o');const inRange=grid[r1+'_'+r2],isHero=label===heroKey;let cls=pp?'pp':'';if(inRange)cls+=' in-range';if(isHero)cls+=' hero-hand';html+=`<td class="${cls.trim()}">${label}</td>`;}html+='</tr>';}return html+'</table>';}
async function newQ(){answered=false;gridVisible=false;document.getElementById('q-area').innerHTML='<div class="loading">Calcul en cours...</div>';try{const r=await fetch('/question');state=await r.json();renderQ();}catch(e){document.getElementById('q-area').innerHTML='<div class="loading" style="color:#f75f5f">Erreur serveur.</div>';}}
function renderQ(){const heroHTML=state.hero.map(c=>cardHTML(c,'hero-c')).join('');const boardHTML=state.board.map(c=>cardHTML(c,'')).join('');const fieldsHTML=state.fields.map((f,i)=>`<div class="field-row"><span class="field-label">${f.label}</span><input class="field-input" id="fi-${i}" type="number" min="0" max="9999" placeholder="?" onkeydown="if(event.key==='Enter')focusNext(${i})"/><span class="field-res" id="fres-${i}"></span><span class="field-ans" id="fans-${i}"></span></div>`).join('');
document.getElementById('q-area').innerHTML=`
  <div style="display:flex;align-items:center;flex-wrap:wrap;gap:5px;margin-bottom:12px;"><span class="badge b-street">${state.street}</span><span class="badge b-sit">${state.situation}</span><span class="badge b-info">${state.total_range_combos} combos dans la range</span></div>
  <div class="main-question"><div class="mq-label">Question</div><div class="mq-text">Combien de combos de la range <b>${state.situation}</b> battent ta main ?</div><div class="mq-sub">Décompose par type de main</div></div>
  <div class="two-col">
    <div>
      <div class="cards-block">
        <div><div class="cards-label">Ta main (hero)</div><div class="cards-row">${heroHTML}</div><div style="margin-top:6px"><span class="badge b-hero">Ta main : ${state.hero_hand}</span></div></div>
        <div><div class="cards-label">Board — ${state.street}</div><div class="cards-row">${boardHTML}</div></div>
      </div>
      <div style="margin-top:10px"><div class="range-toggle"><button onclick="toggleGrid()">Voir la range ▾</button></div>
      <div id="grid-area" style="display:none"><div class="grid-wrap">${buildGrid(state.grid,state.hero_hand_key)}</div>
      <div style="display:flex;gap:12px;margin-top:6px;font-size:10px;color:var(--text3);flex-wrap:wrap;">
        <span><span style="display:inline-block;width:9px;height:9px;background:#1a3a2a;border-radius:2px;vertical-align:middle;margin-right:3px;"></span>Dans la range</span>
        <span><span style="display:inline-block;width:9px;height:9px;outline:2px solid var(--amber);border-radius:2px;vertical-align:middle;margin-right:3px;"></span>Ta main</span>
      </div></div></div>
    </div>
    <div>
